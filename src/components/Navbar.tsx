...
export default function Navbar({ onAuthClick, userEmail }: NavbarProps) {
    ...
    <header className="sticky top-0 z-50 bg-space/80 backdrop-blur-xl border-b border-line-soft">
      <div className="max-w-6xl mx-auto px-5 flex items-center justify-between h-14">
        ...
        <nav className="hidden md:flex items-center gap-6">
          <a href="#features" className="text-sm font-medium text-ink-muted hover:text-ink transition">Features</a>
          <a href="#pricing" className="text-sm font-medium text-ink-muted hover:text-ink transition">Pricing</a>
      ...
      {/* Mobile drawer */}
      <div className={cn('md:hidden fixed inset-0 z-40 transition-opacity', open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none')}>
        ...
        <div className={cn('absolute right-0 top-0 bottom-0 w-[280px] max-w-[80vw] bg-tile border-l border-line p-6 pt-16 transition-transform', open ? 'translate-x-0' : 'translate-x-full')}>
          ...
          <div className="flex flex-col gap-4">
            <a href="#features" onClick={() => setOpen(false)} className="text-base font-semibold text-ink py-2 border-b border-line-soft">Features</a>
            <a href="#pricing" onClick={() => setOpen(false)} className="text-base font-semibold text-ink py-2 border-b border-line-soft">Pricing</a>
            <Link to="/dashboard" onClick={() => setOpen(false)} className="text-base font-semibold text-ink py-2 border-b border-line-soft">Dashboard</Link>
            {userEmail ? (
              <Link to="/dashboard" className="mt-4 px-4 py-2.5 bg-accent text-space text-sm font-bold rounded-full text-center">My Cards</Link>
            ) : (
