...
export default function AuthModal({ open, onClose, onSignInEmail, onSignUpEmail, onSignInGoogle, onLinkGoogle, onSignInAnon, error, isAuthenticated }: AuthModalProps) {
  ...
  return (
    <div className={cn('fixed inset-0 z-[100] flex items-center justify-center px-6 transition-opacity', open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none')}>
      ...
      <div className="relative bg-tile border border-line rounded-2xl p-8 w-full max-w-[420px] shadow-surface">
        ...
        <button onClick={async () => { try { await (isAuthenticated ? onLinkGoogle() : onSignInGoogle()); onClose(); } catch { /* no-op */ } }} className="w-full flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl border border-line bg-transparent text-ink text-sm font-semibold hover:bg-tile-soft transition mb-4">
        ...
        <button onClick={async () => { try { await onSignInAnon(); onClose(); } catch { /* no-op */ } }} className="w-full mt-4 py-2 text-xs text-ink-faint hover:text-ink transition">
